...
public class CustomerServiceImpl extends ServiceImpl<CustomerMapper, Customer> implements CustomerService {
    ...
    public void exportCustomer(CustomerQuery query, HttpServletResponse httpResponse) {
        ...
        List<Customer> customerList = baseMapper.selectList(wrapper);
        ExcelUtils.writeExcel(httpResponse, customerList, "客户信息", "客户列表", CustomerVO.class);
    ...
    public void publicPoolToPrivate(IdQuery idQuery) {
        Customer customer = baseMapper.selectById(idQuery.getId());
        if (customer == null){
            throw new ServerException("客户不存在,无法转回公海");
        }
